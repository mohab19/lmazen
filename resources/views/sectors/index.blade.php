    ...
    <div class="row">
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <div class="page-header">
                ...
                <a href="{{ URL('admin/sectors/create') }}" class="btn btn-primary" id="sample_editable_1_new" style="float: right;">
                                ...
                                <table id="example" class="table table-striped table-bordered second" style="width:100%">
                                    ...
                                    <tbody>
                                        @foreach($sectors as $key => $sector)
                                            <tr class="delete_{{$sector->id}}">
                                                <td>{{$sector->id}}</td>
                                                <td>{{$sector->name}}</td>
                                                <td>{{$sector->sector_type}}</td>
                                                <td>
                                                    <a class="btn btn-primary" href="{{URL('admin/sectors/' . $sector->id)}}" style="padding: 5px 10px;">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    <a class="btn btn-success" href="{{URL('admin/sectors/' . $sector->id . '/edit')}}" style="padding: 5px 10px;">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <span class="delete_this btn btn-danger" data-id="{{$sector->id}}" data-type="Sector" data-url="sectors" style="padding: 5px 10px;">
