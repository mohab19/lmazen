                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <form id="form">
                                ...
                                <input type="hidden" id="route" value="{{route('systems.update', [ app()->getLocale(), $system->id])}}">
                                <div class="form-group">
                                    <label for="name_ar" class="col-form-label">@lang('systems.name_ar')</label>
                                    <input type="text" name="name_ar" class="form-control" value="{{$system->name_ar}}" required>
                                </div>
                                <div class="form-group">
                                    <label for="name_en" class="col-form-label">@lang('systems.name_en')</label>
                                    <input type="text" name="name_en" class="form-control" value="{{$system->name_en}}" required>
                                </div>
                                <select name="type_id" id="type_id"  class="form-control" required>
                                    <option value="0" disabled selected>Select Type: </option>
                                    @foreach($types as $key => $type)
                                    <option value="{{$type->id}}" @if($system->type_id == $type->id) selected @endif>{{$type['name_'.Lang::locale()]}}</option>
                                    @endforeach
                                </select>
                                <div class="form-group">
                                    <label for="support_fees" class="col-form-label">@lang('systems.support_fees')</label>
                                    <input type="text" name="support_fees" class="form-control" value="{{$system->support_fees}}" required>
                                </div>
