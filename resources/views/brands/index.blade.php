    ...
    <div class="row">
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <div class="page-header">
                ...
                <a href="{{ URL( app()->getLocale() . '/admin/brands/create') }}" class="btn btn-primary" id="sample_editable_1_new" style="float: right;">@lang('main.add_new')
                                ...
                                <table id="example" class="table table-striped table-bordered second text-center" style="width:100%">
                                    ...
                                    <tbody>
                                        @foreach($brands as $key => $brand)
                                            <tr class="delete_{{$brand->id}}">
                                                <td>{{$brand->id}}</td>
                                                <td>{{$brand['name_'.Lang::locale()]}}</td>
                                                <td class="@if(Lang::locale() == 'ar') text-left @else text-right @endif">
                                                    <a class="btn btn-primary" href="{{URL( app()->getLocale() . '/admin/brands/' . $brand->id)}}" style="padding: 5px 10px;">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    <a class="btn btn-success" href="{{URL( app()->getLocale() . '/admin/brands/' . $brand->id . '/edit')}}" style="padding: 5px 10px;">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <span class="delete_this btn btn-danger" data-id="{{$brand->id}}" data-type="Brand" data-url="brands" style="padding: 5px 10px;">
